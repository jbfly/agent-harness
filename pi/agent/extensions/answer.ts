 ...
 */
function extractQuestionsFromSourceText(text: string): ExtractionResult | null {
...
function parseExtractionResult(text: string): ExtractionResult | null {
	const normalizeQuestions = (value: unknown): ExtractionResult | null => {
		...
		const normalized = questions
			.map((item): ExtractedQuestion | null => {
				...
				const context = typeof (item as { context?: unknown }).context === "string"
					...
					: undefined;
				return { question, context: context || undefined };
			})
	...
	const questions = text
		.split(/\r?\n/)
		.map((line) => line.replace(/^\s*(?:[-*]|\d+[.)])\s*/, "").replace(/^Q:\s*/i, "").trim())
	...
	const yamlLikeQuestions = [...text.matchAll(/(?:^|\n)\s*[-*]?\s*question\s*:\s*(.+?)(?=\n\s*[-*]?\s*(?:context\s*:|question\s*:)|$)/gim)]
		.map((match) => match[1]?.trim())
		.filter((question): question is string => Boolean(question));
	if (yamlLikeQuestions.length > 0) {
		return {
			questions: yamlLikeQuestions.map((question) => ({ question })),
		};
	}
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
				...
				const doExtract = async () => {
					...
					if (!auth.ok || !auth.apiKey) {
						return {
							...
							message: auth.ok ? `No API key for ${extractionModel.provider}/${extractionModel.id}` : auth.error,
						};
					}
					...
					const parsed = parseExtractionResult(responseText) ?? extractQuestionsFromSourceText(lastAssistantText!);
					if (!parsed) {
						return {
							status: "error" as const,
							message: "Question extraction returned an unexpected format",
							rawText: responseText,
						};
					}

					return { status: "ok" as const, result: parsed };
				};
			...
			if (extractionOutcome.status === "error") {
				...
				if (extractionOutcome.rawText) {
					ctx.ui.setEditorText(extractionOutcome.rawText);
					ctx.ui.notify("Raw extraction output loaded into editor for debugging", "info");
				}
				return;
			}
